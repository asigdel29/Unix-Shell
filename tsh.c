 ...
 * Anubhav Sigdel - asigdel@oswego.edu
 */
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
void eval(char *cmdline)
{
    ...
    bg = parseline(cmdline, argvector);
    if(argvector[0] == NULL)
        return;

    if(!builtin_cmd(argvector)){
        ...
        if((pid = fork()) < 0)
            ...
            if(execve(argvector[0], argvector, environ) < 0){
...
void do_bgfg(char **argv)
{
    ...
    if(argv[1] == NULL){
        printf("%s requires PID or argument\n", argv[0]);
        return;
    }
    ...
    if(strcmp(argv[0], "fg")){
        if(stringID[0] == '%'){
            ...
            printf("%s: must be a PID or jobid\n", argv[0]);
        }
    } else if(strcmp(argv[0], "bg")){
        if(stringID[0] == '%'){
            jid  = atoi(&stringID[1]);
...
void waitfg(pid_t pid)
    {
        while (1) {
            if (pid != fgpid(jobs)) {
                if (verbose)
                    printf("waitfg: process (%d) no longer a forground process\n", (int)pid);
                break;
...
void sigchld_handler(int sig)
{
    pid_t pid;
    int jobid, jobstatus;
    while((pid = waitpid(-1, &jobstatus, WNOHANG | WUNTRACED)) > 0) {
        jobid = pid2jid(pid);
        if (WIFEXITED(jobstatus)) {
            ...
            if (verbose)
                printf("sigchld_handler: Job [%d] (%d) terminates OK (status %d)\n", jobid, (int) pid, WEXITSTATUS(jobstatus));
        }
        else if (WIFSIGNALED(jobstatus)) {
            deletejob(jobs,pid);
            if (verbose)
                printf("sigchld_handler: Job [%d] (%d) deleted\n", jobid, (int) pid);
            printf("Job [%d] (%d) terminated by signal %d\n", jobid, (int) pid, WTERMSIG(jobstatus));
        }
        else if (WIFSTOPPED(jobstatus)) {
            getjobpid(jobs, pid)->state = ST;
            printf("Job [%d] (%d) stopped by signal %d\n", jobid, (int) pid, WSTOPSIG(jobstatus));
        }
